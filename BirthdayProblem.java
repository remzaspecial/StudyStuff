...
public class Logic
    ...
    public double calculate(int size, int count) {
        int SuccessOnes = successOnes(count, size);
        double result = (double) SuccessOnes * 100 / count; //formula was taken from unitTest
        return result;
    }
    //when rand.nextInt gives us duplicate , if! statement works and makes successOnes ++;
    //here is my very easy solution of our problem , thank you guys:)
    public static int successOnes(int simulations, int groupSize) {
        int successOnes = 0; // counter for simulations with duplicate birthdays
        for (int i = 0; i < simulations; i++) {
            Random rand = new Random(i+1);
            HashSet set = new HashSet(); // we need HashSet cuz in set we can't add duplicate numbers
            for (int j = 0; j < groupSize; j++) {
                if (!set.add(rand.nextInt(365))) {//the if statement checks for duplicate
                    successOnes++;
                    break;
                }
            }
